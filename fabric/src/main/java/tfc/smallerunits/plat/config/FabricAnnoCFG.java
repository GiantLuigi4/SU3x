...
public class FabricAnnoCFG extends AnnoCFG {
    private final HashMap<String, ConfigEntry> handles = new HashMap<>();

    private static final ArrayList<FabricAnnoCFG> configs = new ArrayList<>();

    Config internal;

    public FabricAnnoCFG(Class<?> clazz) {
        tfc.smallerunits.plat.config.annoconfg.annotation.format.Config configDescriptor =
                clazz.getAnnotation(tfc.smallerunits.plat.config.annoconfg.annotation.format.Config.class);
