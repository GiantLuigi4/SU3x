	...
	public FabricPacketRegister(
		...
		if (PlatformProvider.UTILS.isClient()) {
			ClientPlayNetworking.registerGlobalReceiver(
					...
					((client, handler, buf, responseSender) -> {
						handlePacket(handler, buf, new tfc.smallerunits.plat.net.PacketSender(this, (pkt) -> responseSender.sendPacket(name, encode(pkt))), ((PacketListenerAccessor) handler).getPlayer(), NetworkDirection.TO_CLIENT);
					})
			);
		}
		ServerPlayNetworking.registerGlobalReceiver(
				...
				((server, player, handler, buf, responseSender) -> {
					handlePacket(handler, buf, new tfc.smallerunits.plat.net.PacketSender(this, (pkt) -> responseSender.sendPacket(name, encode(pkt))), player, NetworkDirection.TO_SERVER);
				})
		);
	}
...
//	public <T extends Packet> void registerMessage(int indx, Class<T> clazz, BiConsumer<Packet, FriendlyByteBuf> writer, Function<FriendlyByteBuf, T> fabricator, BiConsumer<Packet, FabricNetCtx> handler) {
//
//	}
