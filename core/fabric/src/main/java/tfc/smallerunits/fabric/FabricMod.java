...
public class FabricMod extends SmallerUnits implements ModInitializer {
	...
	public void registerSetup(Runnable common) {
		if (PlatformProvider.UTILS.isClient()) ClientLifecycleEvents.CLIENT_STARTED.register((a) -> common.run());
	...
	public void onWorldUnload(Consumer<LevelAccessor> consumer) {
		if (PlatformProvider.UTILS.isClient()) {
			// TODO: hook into some places?
			// clearLevel seems like a good hook point
			// TODO: cleaner-based fallback?
			System.err.println("WARN: NYI, world unload event on client for fabric");
		}
		ServerWorldEvents.UNLOAD.register((srv, lvl) -> {
			consumer.accept(lvl);
		});
	}
}
